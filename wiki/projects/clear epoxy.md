# Clear Epoxy

**Type:** Project
**Tags:** #project #craft #resin
**Status:** Research phase

---

## Overview

Get into clear epoxy casting. Research the process, order supplies, start experimenting.

## Casting vs. Epoxy Resin

**Casting resin:** Lower viscosity, designed for deep pours and molds. Slower cure (less heat = fewer bubbles in deep pours). Used for: jewelry, figures, embedded objects, thick river tables.

**Epoxy resin:** Higher viscosity, designed for surface coating (tabletops, bar tops). Cures faster, self-levels. Not designed for thick pours — generates too much heat and can crack or yellow.

**Floating objects in epoxy:**
Pour in layers. First layer cures to a "gel" stage (tacky but not liquid) → place your object → pour remaining layers on top. This suspends the object mid-resin. If you pour while still fully liquid, the object will sink or float to the surface depending on density. Timing the gel stage is key — usually a few hours in.

---

## Research Checklist

- [ ] Mold types — silicone, plastic, wood forms, custom-made
- [ ] Layer depth limits — most epoxy max pour is ¼"–½" per layer (heat buildup if too thick)
- [ ] Temperature — ambient vs. heat-cured; warmer = faster cure, can cause bubbles if too hot
- [ ] Cure time — working time (minutes) vs. full cure (typically 24–72h depending on product)
- [ ] Cost — resin + hardener ratio (typically 1:1 or 2:1 by volume), cost per pour
- [ ] Brands — Total Boat, ArtResin, Pro Marine Supplies, Stone Coat

## Potential Uses

- Embedded objects (flowers, photos, pressed wood grain)
- River table surfaces
- Decorative cast pieces / gifts

## Notes

- **Lesson learned (2026-07-25):** this batch hardened in ~2 hours — much faster than expected. Left it sitting for the full 2 hours before checking, and everything embedded sank straight through to the bottom of the brick. The gel-stage window (see above) closes fast — layer *well before* the 2-hour mark, not after it.

---

## Related

- [[wood cutting board]]
- [[wooden keyboard]]
